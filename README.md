...
3.压缩算法的考量，相关性那里计算用score/max_score,热度是a*log(1+score)/log(1+p99)+b*log(1+score)/log(1+p99)，这里用p99主要是做了个上限，上限的意义是我们不太想让热度分数随着热度无限上涨是有上限的
p99合不合适，引来的成本p99用快照还是实时，实时遍历复杂度还好但有实实在在的开销  
4.分页的展示，调研成果  
5.硬编码的召回和rerank是否粗糙  
6.bm25打的这个分我们应用层除了归一化要不要再处理给个基准分动态可调再去加分减分，三者都归一化合适吗
