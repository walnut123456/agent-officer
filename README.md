...
## 多维度同一尺度打分细节
page*size*k粗召回，然后再精排取对应页码的
相关性那里用score/max_score,因为bm25计算已经做了饱和处理
热度是a*log(1+score)/log(1+p99)+b*log(1+score)/log(1+p99)

1.为什么不把热度和时间直接在es里和相关性统一打分？

2.跨分页不同次访问接口，即时热度可能不统一导致排序乱的问题？  
3.压缩算法的考量，这里用p99主要是做了个上限，上限的意义是我们不太想让热度分数随着热度无限上涨是有上限的
p99合不合适，引来的成本p99用快照还是实时，实时遍历复杂度还好但有实实在在的开销  

6.tob项目引入这一套统一尺度打分合不合理        
因为我们不想让比如相关性接近但热度区分大的合理把他们区分出来，而非同时只有一个分数为主导  
