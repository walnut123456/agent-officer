...
public class TaskJobAutoConfig {
    ...
    public ITaskJobService taskJobService(@Qualifier("xfgWrenchTaskScheduler") TaskScheduler xfgWrenchTaskScheduler,
        ...
        return new TaskJobService(xfgWrenchTaskScheduler, taskDataProviders);
    }

    @Bean
    public ApplicationRunner taskJobInitializer(ITaskJobService taskJobService) {
        return args -> {
            log.info("应用启动完成，开始延迟初始化任务调度配置");
