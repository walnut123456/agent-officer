...
import org.wwz.ai.domain.agent.runtime.tool.resilience.ToolResiliencePipeline;
...
public class ToolCollection {
     ...
     * 工具调用韧性拦截链。
     * ToolCollection 不是 Spring Bean，因此必须由外部显式注入。
     * 为 null 时回退到直接调用（无韧性保护）。
     */
    @ToString.Exclude
    @EqualsAndHashCode.Exclude
    @JSONField(serialize = false, deserialize = false)
    private ToolResiliencePipeline resiliencePipeline;
    ...
    public Object execute(String name, Object toolInput) {
        ...
        Supplier<Object> actualCall = null;

        // 分支1：执行本地基础工具
        if (toolMap.containsKey(name)) {
            BaseTool tool = getTool(name);
            idempotent = tool.isIdempotent();
            actualCall = () -> tool.execute(toolInput);
        }
        // 分支2：执行远程MCP工具
            ...
            idempotent = Boolean.TRUE.equals(toolInfo.getIdempotent());
            actualCall = () -> executor.executeTool(toolInfo, toolInput);
        }
        // 分支3：工具不存在，记录错误日志
        else {
        ...
        if (resiliencePipeline != null) {
            return resiliencePipeline.execute(name, idempotent, actualCall);
        }
