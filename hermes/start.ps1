...
if (-not (Test-Path $pythonExe)) {
    throw "Missing local virtual environment: $pythonExe. Run 'uv sync' in hermes first."
}
...
try {
    ...
    if ($listener) {
        ...
        if ($isCurrentServer) {
            Write-Host "Hermes Officer is already running on port $port (PID $($listener.OwningProcess))."
            Write-Host "Close the existing Hermes Officer window first if you want to restart it."
            exit 0
        }
