...
from hermes_officer.tool.table_rag.es_client import ElasticsearchClient
from hermes_officer.tool.table_rag.retriever import Retriever


class ElasticsearchClientAuthTest(unittest.TestCase):

    def test_should_use_api_key_for_cloud_es(self):
        with patch("hermes_officer.tool.table_rag.es_client.Elasticsearch") as elasticsearch_cls:
            ElasticsearchClient(
                {
    ...
    def test_retriever_should_read_data_agent_es_config_only(self):
        with patch.dict(
            os.environ,
            {
                ...
                "TR_ES_CONFIGS_INDEX": "hermes_model_column_value",
            },
            clear=True,
        ):
            with patch("hermes_officer.tool.table_rag.retriever.ElasticsearchClient") as elasticsearch_client_cls:
        ...
        self.assertEqual("hermes_model_column_value", retriever.es_index)
    ...
    def test_should_retry_without_analyzer_when_ik_plugin_is_missing(self):
        ...
        with patch("hermes_officer.tool.table_rag.es_client.Elasticsearch", return_value=fake_client):
