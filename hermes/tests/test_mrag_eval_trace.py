...
from hermes_officer.tool.mrag.query.aigent import AgenticRAG
...
class AgenticRagEvalTraceTest(unittest.TestCase):
    ...
    def test_should_collect_trace_for_retrieval_backed_query(self):
        ...
        with patch(
            "hermes_officer.tool.mrag.query.aigent.QueryProcessor.simple_query_check",
            return_value=False,
        ), patch(
            "hermes_officer.tool.mrag.query.aigent.QueryProcessor.extend_questions",
            return_value=["子问题1"],
        ), patch(
            "hermes_officer.tool.mrag.query.aigent.QueryProcessor.summarize_subquery",
            return_value="足够了",
        ), patch(
            "hermes_officer.tool.mrag.query.aigent.QueryProcessor.generate_next_instruction",
            ...
            "hermes_officer.tool.mrag.query.aigent.get_text_reranker",
        ) as reranker_factory:
    ...
    def test_should_keep_simple_image_query_fast_path_without_retrieval_trace(self):
        ...
        with patch(
            "hermes_officer.tool.mrag.query.aigent.QueryProcessor.extract_image_content",
            return_value="这是一只猫",
        ), patch(
            "hermes_officer.tool.mrag.query.aigent.QueryProcessor.simple_query_check",
            return_value=False,
        ), patch(
            "hermes_officer.tool.mrag.query.aigent.QueryProcessor.simple_image_query_check",
            return_value=True,
        ), patch.object(
            agent,
            "vlm_answer",
