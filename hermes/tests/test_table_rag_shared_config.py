...
from hermes_officer.tool.table_rag.qdrant_recall import get_qd_recall
from hermes_officer.util.qdrant_utils import resolve_table_rag_qdrant_config
...
class TableRagSharedConfigTest(unittest.TestCase):
    ...
    def test_should_keep_legacy_embedding_url_override(self):
        with patch.dict(os.environ, {"TR_EMBEDDING_URL": "http://legacy.local/embedding"}, clear=True):
            with patch("hermes_officer.tool.table_rag.qdrant_recall.EmbeddingClient") as embedding_client_cls:
                with patch("hermes_officer.tool.table_rag.qdrant_recall.get_text_embedding_model") as get_text_embedding_model:
                    with patch("hermes_officer.tool.table_rag.qdrant_recall.QdrantRecall") as qdrant_recall_cls:
