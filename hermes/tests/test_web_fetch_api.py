...
from hermes_officer.api.tool import router
from hermes_officer.tool.web_fetcher import WebFetchResult
...
class WebFetchApiTest(unittest.TestCase):
    ...
    def test_should_return_unified_response_and_file_info(self):
        ...
        with patch("hermes_officer.api.tool.WebFetcher.fetch", new=AsyncMock(return_value=fetch_result)):
            with patch(
                "hermes_officer.api.tool.upload_file",
                new=AsyncMock(
                    return_value={
                        "fileName": "测试网页.md",
    ...
    def test_should_return_400_when_fetcher_rejects_request(self):
        with patch("hermes_officer.api.tool.WebFetcher.fetch", new=AsyncMock(side_effect=ValueError("web_fetch 仅支持 HTML、Markdown 或纯文本内容"))):
            response = self.client.post(
                "/v1/tool/web_fetch",
                json={
