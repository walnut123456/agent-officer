...
from hermes_officer.model.code import ActionOutput, CodeOuput
from hermes_officer.model.protocal import (
    TableRAGRequest,
    AutoAnalysisRequest,
    CIRequest,
    CalEngineRequest,
...
from hermes_officer.tool.code_interpreter_policy import CodeExecutionPermissionError
...
from hermes_officer.util.middleware_util import RequestHandlerRoute
load_dotenv()
...
def _get_code_interpreter_agent():
    """Resolve the optional heavy runtime only when this endpoint is called."""
    from hermes_officer.tool.code_interpreter import code_interpreter_agent

    return code_interpreter_agent
...
async def post_code_interpreter(
    ...
    code_interpreter_agent = _get_code_interpreter_agent()
...
async def post_mrag_query(body: MultimodalRAGRequest):
    ...
    # 兼容 OpenAI 兼容网关与 DashScope 两种配置方式，只要任一可用即可。
    embedding_api_key = (
        os.getenv("IMAGE_EMBEDDING_API_KEY")
        or os.getenv("TEXT_EMBEDDING_API_KEY")
    ...
    if not kb_id or not qdrant_url or not embedding_api_key:
        missing = []
        if not kb_id:
            missing.append("DEFAULT_KB_ID")
        if not qdrant_url:
            missing.append("QDRANT_URL")
        if not embedding_api_key:
            missing.append("IMAGE_EMBEDDING_API_KEY 或 OPENAI_API_KEY")
        error_msg = f"多模态检索服务配置不完整，缺少环境变量：{', '.join(missing)}。请在 .env 文件中配置后重试。"
        return EventSourceResponse(
            [json.dumps(_build_mrag_chunk(error_msg, "stop"), ensure_ascii=False), "[DONE]"],
            ping_message_factory=lambda: ServerSentEvent(data="heartbeat"),
            ping=15,
        )
