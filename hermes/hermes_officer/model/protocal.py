...
class CIRequest(BaseModel):
    ...
    file_names: Optional[List[str]] = Field(default_factory=list, alias="fileNames", description="输入的文件列表")
    ...
    stream: bool = True
    stream_mode: Optional[StreamMode] = Field(default_factory=StreamMode, alias="streamMode", description="流式模式")
...
class DeepSearchRequest(BaseModel):
    ...
    search_engines: List[str] = Field(default_factory=list, description="使用哪些搜索引擎")
...
class SopChooseRequest(BaseModel):
    request_id: str = Field(alias="requestId", description="Request ID")
    query: str = Field(description="用户问题")
    sop_list: Optional[List[Dict]] = Field(default_factory=list,
