...
from hermes_officer.infrastructure.database import Database, FileInfoRecord
from hermes_officer.util.log_util import timer


_database: Database | None = None


def configure_file_database(database: Database) -> None:
    """Bind the legacy-compatible file API to the application's primary DB."""
    global _database
    _database = database
...
def _session():
    if _database is None:
        raise RuntimeError("File metadata database has not been configured")
...
class _FileDB(object):
    ...
    async def save(self, file_name, content, scope) -> str:
        ...
        # `scope` can contain ":" (e.g. "hermessession-...:..."), which causes
...
class FileInfoOp(object):
    ...
    async def add_by_content(cls, filename: str, content: str, file_id: str, description: str = None,
                             request_id: str = None) -> FileInfoRecord:
    ...
    @staticmethod
    @timer()
    async def add_by_file(file: UploadFile, file_id: str, request_id: str = None) -> FileInfoRecord:
    ...
    @staticmethod
    @timer()
    async def add(file_info: FileInfoRecord) -> FileInfoRecord:
    ...
    @staticmethod
    @timer()
    async def get_by_file_id(file_id: str) -> FileInfoRecord | None:
        async with _session() as session:
            state = select(FileInfoRecord).where(FileInfoRecord.file_id == file_id)
            result = await session.execute(state)
            return result.scalars().one_or_none()

    @staticmethod
    @timer()
    async def get_by_file_ids(file_ids: List[str]) -> List[FileInfoRecord]:
        async with _session() as session:
            state = select(FileInfoRecord).where(FileInfoRecord.file_id.in_(file_ids))
            result = await session.execute(state)
            return result.scalars().all()

    @staticmethod
    @timer()
    async def get_by_request_id(request_id: str) -> List[FileInfoRecord]:
        async with _session() as session:
            state = select(FileInfoRecord).where(FileInfoRecord.request_id == request_id)
            result = await session.execute(state)
            return result.scalars().all()
