...
def prepare_workspace(skill_base_path: str, arguments: Dict) -> PreparedWorkspace:
    ...
    # Windows may expose the temp directory through an 8.3 short path while
    # child.resolve() returns the long path. Canonicalize both before doing
    # containment checks, otherwise valid scripts are rejected as traversal.
...
def _resolve_project_python() -> str | None:
    """优先返回 hermes 本地 .venv 的 Python，避免被外层启动器解释器影响。"""
