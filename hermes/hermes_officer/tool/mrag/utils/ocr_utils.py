...
def get_ocr_model() -> OCRBase:
    ...
    if ocr_type == "deepseek-ocr":
        ocr = DeepSeekOCR()
        if not ocr.api_key or not ocr.base_url or not ocr.model:
            logger.warning(
                "OCR_TYPE=deepseek-ocr 但 DEEPSEEK_OCR_* 配置不完整，自动回退到 vlm-ocr"
            )
            return VLMOCR()
        return ocr
        ...
        logger.warning(f"未配置有效的 OCR_TYPE，自动回退到 vlm-ocr: {ocr_type}")
        return VLMOCR()
