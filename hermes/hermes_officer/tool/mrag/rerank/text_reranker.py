...
class APITextReranker(TextReranker):
    ...
    def __init__(self):
        self.headers = {
            ...
            "Authorization": "Bearer " + (os.getenv("TEXT_RERANKER_API_KEY") or ""),
        }
        ...
        # TEXT_RERANKER_BASE_URL 应为 base_url，自动拼接 /rerank 接口路径。
        base_url = (os.getenv("TEXT_RERANKER_BASE_URL") or "").rstrip("/")
    ...
    def _prepare_request_data(self, question: str, texts: list[str]) -> Dict[str, Any]:
        ...
        # 使用扁平格式（query/documents 在顶层），兼容 BigModel / OpenAI 兼容网关。
        return {
            "model": self.model,
            "query": question,
...
def get_text_reranker() -> TextReranker:
    text_reranker_type = (os.getenv("TEXT_RERANKER_TYPE") or "").strip().lower()
    if text_reranker_type == "api":
