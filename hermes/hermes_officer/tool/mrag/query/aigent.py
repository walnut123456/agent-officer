...
class AgenticRAG:
    ...
    def __init__(self, kb_id: str, n_round: int = 5):
        ...
        # Embedding/rerank models may require sizeable local artifacts. Delay
        # construction until retrieval is actually requested so health checks,
        # simple-query fast paths and unit tests never trigger model downloads.
    ...
    def retrieval(self, questions: list[str]) -> List[List[Dict]]:
        text_resp = self._get_retriever().retrieval_by_texts(self._kb_id, questions)
        return text_resp
