...
from hermes_officer.util.log_util import AsyncTimer
...
class UnknownException(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        try:
            ...
            logger.exception(
                "legacy middleware error request_id={} method={} path={}",
                RequestIdCtx.request_id,
                request.method,
                request.url.path,
            )
            return Response(content="Service temporarily unavailable", status_code=500)
    ...
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request) -> Response:
            try:
                ...
                if (
                    os.getenv("LOG_REQUEST_BODIES", "false").lower() == "true"
