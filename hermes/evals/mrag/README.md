...
这套工具用于评估 `hermes` 当前 MRAG 检索链路的 evidence recall，而不是最终答案质量。
...
```bash
uv run python -m hermes_officer.tool.mrag.eval.cli export-manifest --kb-id <kb_id> --output evals/mrag/manifest.jsonl
```
...
```bash
uv run python -m hermes_officer.tool.mrag.eval.cli recall --kb-id <kb_id> --dataset-dir evals/mrag/sample
```
