...
/**
 * 剥离后端总结 Agent 可能残留的结构化 XML 标签。
 * 仅处理已知的总结标签（final_answer、core_view 等），不影响合法 HTML。
 */
const SUMMARY_XML_TAG_PATTERN =
  /<\/?\s*(final_answer|core_view|text|reasoning|key_points|summary|todo|artifact_info)\s*>/gi;

function stripSummaryXmlTags(content: string): string {
  return content.replace(SUMMARY_XML_TAG_PATTERN, "");
}
...
export function normalizeMarkdownForDisplay(
  ...
  const normalizedText = stripSummaryXmlTags(content)
    .replace(/\uFEFF/g, "")
    .replace(/\r\n?/g, "\n");
